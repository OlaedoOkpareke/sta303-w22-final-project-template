...
To create the main dataset, we merged the customer information data, the sleep data, and the customer device data. To create the possible explanatory models, we created an age variable by subtracting the date of birth of the individual from the date of analysis; March 30th 2022. We renamed the emoji variables to understandable skin tones, and removed all missing values in this variable as we could not use the missing values in analysis. We dropped all missing values for the sex variable to make the two possible explanatory models formed comparable in size. we rescaled the age variable to be confined only to values within 0 and 1, which was done to solve errors in creating the model. 
