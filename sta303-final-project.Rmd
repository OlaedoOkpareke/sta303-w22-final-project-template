...
MINGAR has received complaints about the fact that their devices have not been working properly for individuals with darker skins with respect to sleep scores. Therefore, the study investigates whether the different MINGAR lines of devices were discriminatory against particular skin tones with respect to these sleep scores. 
We use a measure of the devices’ performance called flags, which is given a rate by the minutes of sleep. A higher quantity of flags per minute of sleep insinuates a worse performance of device used during the sleep, which in turn means poor sleep score reliability. We find that individuals with darker skin tones have a higher flag count per minute, therefore a worse sleep score, than users with lighter skin tones. 

## Key findings

* From Table 1, we estimate that on average, the number of flags per minute grows as we darken the skin tone. The confidence interval, which is the range of plausible values the mean number of flags can take, confirms this. Therefore, we observe significant differences between skin tones in MINGAR’s device sleep performance. 
...
Our final model is $$ log(\lambda)_j = \beta_0 + \beta_1\alpha_{1ij} + \beta_2\alpha_{2ij} + \gamma_{ij} + log(duration)_{ij} $$ 

*how do i write this model? is there supposed to be an error term?*
Where:

y_ij tilda pois(lambda)
* $\lambda$ is the mean number of flags for day j of sleep

* $\alpha_{1ij}$ is the skin tone of person day j. This is a fixed effect. 

* $\alpha_{2ij}$ is the age of person on day j. This is also a fixed effect.
...
* $duration$ i and j is offset by duration, to account for the different minutes of sleep people have, so they may have different rates of flags. 
