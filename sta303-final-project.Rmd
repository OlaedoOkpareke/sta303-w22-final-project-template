...
We see that the mean and variance of darker skinned people tends to be more than light. This indicates that there might be a problem with race for them. we see that both mean count and mean rate for darker skin tones is high. mean and variance are pretty close, so we can assume that this assumption is satisfied for the most part
...
It is offset by duration, to account for the different minutes of sleep peopke have, so they may have different rates of flags
...
This figure shows the estimated value and the confidence intervals. The y is log due to the poisson nature if the response. As stated earlier, age is also significant we see this by. we see young are more likely to get flagged
