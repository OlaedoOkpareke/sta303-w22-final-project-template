...
summary(modrace)
# why did the na's drop? how do i include them in my model?
...
```
Using both anova and lmtest, we can reject the null that modrace is just as goof as modrace2. We have modrace2 as final model. 
...
```
```{r}
(Confint = confint(modrace2))
```

# proportion table of all respondents that have from 0-10 flags in their sleep, that wear each device, etc


# bar charts showing the differences between sub populations
```{r}
...
```{r}
cust_full %>%
  ggplot(data =., aes(x=duration, fill = emoji_modifier)) +
  geom_histogram()+ 
  theme_minimal()
```
...
```{r}
cust_full %>%
  ggplot(data =., aes(x=age, fill = emoji_modifier)) +
  geom_histogram()+ 
  theme_minimal()
```
what effect does this say about flags? young are more likely to get flagged?


```{r}
with(cust_full,round(prop.table(table(emoji_modifier,sex),1),3)) %>% 
